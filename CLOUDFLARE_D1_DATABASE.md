...
```bash
npx wrangler d1 execute slgrvtrs-voters --remote --file=./schema.sql
```

### 6.4 Load Pre-Aggregated Stats (Phase 2-3)

```bash
# Convert existing JSON stats to SQL INSERT statements
python3 scripts/build_d1_stats.py > stats_load.sql
npx wrangler d1 execute slgrvtrs-voters --remote --file=./stats_load.sql
```
...
2. `wrangler login` — Authenticate (browser OAuth, no credit card)
3. `npx wrangler d1 create slgrvtrs-voters` — Create database
4. Save `database_id` to `wrangler.jsonc`
5. Create `schema.sql` with the table definitions above
6. `npx wrangler d1 execute slgrvtrs-voters --remote --file=./schema.sql`
7. Run Python scripts to generate SQL from Excel data
8. Load in batches: `npx wrangler d1 execute slgrvtrs-voters --remote --file=./batch.sql`
9. Verify: `npx wrangler d1 execute slgrvtrs-voters --remote --command="SELECT COUNT(*) FROM dms"`
