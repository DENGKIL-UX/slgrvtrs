...
| `POST /api/export/dm-voters/[dm_code]` | individual voters | per DM (on-the-fly from D1) | ~4,203 avg |

All endpoints use the same password: `PAStimenang1` (PBKDF2, 10K iterations)

**Note**: The `/api/export/dm-voters/[dm_code]` endpoint generates voter records
on-the-fly from the D1 database (no R2 pre-generated files needed). It queries
the DM's gender×race crosstabs and generates synthetic voter records.
